...
if (argument !== 'install' || !fs.existsSync(path.resolve('.', 'package.json')) && !process.argv[3]) {
  ...
  run('npm init -y', targetFolder)
    ...
    .then(() => {
      ...
      fs.copyFileSync(path.resolve(__dirname, 'setup/standalones.js'), path.resolve(targetFolder, 'configuration', 'standalones.js'))
      fs.copyFileSync(path.resolve(__dirname, 'setup/packages.js'), path.resolve(targetFolder, 'configuration', 'packages.js'))
      fs.copyFileSync(path.resolve(__dirname, 'setup/providers.js'), path.resolve(targetFolder, 'configuration', 'providers.js'))
